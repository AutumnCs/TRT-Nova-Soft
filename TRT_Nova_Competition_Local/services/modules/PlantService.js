...
class PlantService {
  ...
  async hydratePlantPets(pets = []) {
    const source = Array.isArray(pets) ? pets : [];
    const fileIds = source.map((item) => item.coverFileId).filter(Boolean);
    const displayMap = fileIds.length ? await mediaStorageService.resolveFileIds(fileIds) : {};
    return source.map((item) => ({
      ...item,
      coverUrl: displayMap[item.coverFileId] || item.coverUrl || ''
    }));
  }

  async listMyPlantPets(options = {}) {
    const result = await this.scfApiAdapter.listPlantPets({
      includeArchived: options.includeArchived === true
    });
    return {
      success: result?.success !== false,
      pets: await this.hydratePlantPets(result?.pets),
      msg: result?.msg || ''
    };
  }

  async getMyPlantPet(plantPetId) {
    const result = await this.scfApiAdapter.getPlantPet(plantPetId);
    const pets = result?.pet ? await this.hydratePlantPets([result.pet]) : [];
    return { ...result, pet: pets[0] || null };
  }

  async createPlantPet(payload = {}) {
    const result = await this.scfApiAdapter.createPlantPet(payload);
    const pets = result?.pet ? await this.hydratePlantPets([result.pet]) : [];
    return { ...result, pet: pets[0] || null };
  }

  async updatePlantPet(payload = {}) {
    const result = await this.scfApiAdapter.updatePlantPet(payload);
    const pets = result?.pet ? await this.hydratePlantPets([result.pet]) : [];
    return { ...result, pet: pets[0] || null };
  }

  async archivePlantPet(plantPetId) {
    return this.scfApiAdapter.archivePlantPet(plantPetId);
  }

  async deletePlantPet(plantPetId) {
    return this.scfApiAdapter.deletePlantPet(plantPetId);
  }
