...
function normalizeArticle(article = {}) {
  return {
    ...
    sourceType: String(article.sourceType || article.source_type || 'external-reference').trim() || 'external-reference',
    sourceRef: String(article.sourceRef || article.source_ref || '').trim(),
    sourceTitle: String(article.sourceTitle || article.source_title || '').trim(),
    sourcePublisher: String(article.sourcePublisher || article.source_publisher || '').trim(),
    sourceUpdatedAt: String(article.sourceUpdatedAt || article.source_updated_at || '').trim(),
    sourceUrl: String(article.sourceUrl || article.source_url || '').trim(),
    sourceId: String(article.sourceId || article.source_id || '').trim(),
    contentUpdatedAt: article.contentUpdatedAt || article.content_updated_at || null,
    reviewedAt: article.reviewedAt || article.reviewed_at || null,
    reviewedBy: String(article.reviewedBy || article.reviewed_by || '').trim(),
    imageLicense: String(article.imageLicense || article.image_license || '').trim(),
    imageSourceUrl: String(article.imageSourceUrl || article.image_source_url || '').trim(),
...
export function createKnowledgeService({ repository, seedArticles = [] } = {}) {
  ...
  return {
    ...
    async saveArticle(article) {
      ...
      if (!['draft', 'reviewed', 'published'].includes(normalized.status)) throw new Error('invalid knowledge status');
      if (normalized.status === 'published' && (!normalized.reviewedAt || !normalized.reviewedBy)) {
        throw new Error('published knowledge requires reviewedAt and reviewedBy');
      }
      return { success: true, article: normalizeArticle(await repository.saveArticle(normalized)), source: 'database' };
    },
