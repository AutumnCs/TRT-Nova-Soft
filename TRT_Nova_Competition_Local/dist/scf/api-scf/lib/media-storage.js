...
async function uploadLocalMediaForUser(db, openid, input = {}) {
  const cloud = process.env.MEDIA_STORAGE_PROVIDER === 'cos';
  ...
  if (cloud) {
    const media = await require('../cloud-media').upload(db, openid, value);
    return { success: true, media: { ...media, extension: MIME_EXTENSIONS[value.mimeType] } };
  }
...
async function resolveLocalMediaForUser(db, openid, input = {}) {
  ...
  if (process.env.MEDIA_STORAGE_PROVIDER === 'cos' && requested.length > 1) {
  ...
  const media = await Promise.all(requested.filter((fileId) => byFileId.has(fileId)).map(async (fileId) => {
    ...
    return {
      ...
      contentBase64: row.provider === 'cos'
...
async function claimLocalMediaFiles(db, openid, fileIds, options = {}) {
  ...
  if (process.env.MEDIA_STORAGE_PROVIDER === 'cos' &&
      (normalizedIds.length !== new Set(supplied).size || normalizedIds.some(id => !id.startsWith('cos://')))) {
...
function isManagedFileId(fileId) {
  return isLocalFileId(fileId) || /^cos:\/\/[a-f0-9-]{36}$/i.test(String(fileId || ''));
}
