...
async function loadOwnedConversationDocument(db, openid, mediaFileId, plantPetId, lock = false) {
  ...
  if (fileId.startsWith('cos://') && process.env.MEDIA_STORAGE_PROVIDER === 'cos' && !lock) {
    media.content_blob = await require('../cloud-media').read(db, openid, fileId);
  }
  if (!media.content_blob && !(lock && fileId.startsWith('cos://') && process.env.MEDIA_STORAGE_PROVIDER === 'cos')) {
