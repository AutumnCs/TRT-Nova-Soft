...
test('reports no errors on the current repository layout', async () => {
  ...
  // A fresh source delivery has canonical skill files, not machine-specific junctions.
  assert.ok(result.warnings.every(message => message.startsWith('cross-tool junctions missing')));
});
