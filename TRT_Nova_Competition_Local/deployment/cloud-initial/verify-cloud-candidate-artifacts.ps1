...
$requiredEntries = @('index.js', 'cloud-entry.js', 'package.json', 'package-lock.json', 'NOVA_PACKAGE_MANIFEST.json')
...
foreach ($functionName in $functionNames) {
  ...
  try {
    ...
    if ($null -eq $manifestEntry) {
      ...
      if ([string]$manifest.handler -ne 'cloud-entry.main_handler') {
        $manifestIssues += 'manifest handler is not cloud-entry.main_handler'
      }
