...
import { cloudRoot, repoRoot, loadPolicy, collectFirstPartyFiles, sha256File } from './common.mjs';
...
for (const unit of policy.functions) {
  ...
  for (const name of ['cloud-entry.js', 'cloud-secrets.js',
    ...(['api-scf', 'agent-scf', 'history-cleanup-scf'].includes(unit.name) ? ['cloud-media.js'] : [])]) {
