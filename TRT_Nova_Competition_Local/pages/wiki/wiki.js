...
Page({
  ...
  decoratePlants(plants = []) {
    return (Array.isArray(plants) ? plants : []).map((item) => {
      const issue = Array.isArray(item.commonIssues) && item.commonIssues.length
        ? item.commonIssues[0]
        : null;
      return {
        ...item,
        viewId: String(item.id || item.sourceId || item.name || ''),
        displayImage: item.image || '/images/plant-default-v2.jpg',
        firstIssue: issue ? {
          ...issue,
          possibleCausesText: Array.isArray(issue.possibleCauses) ? issue.possibleCauses.join('、') : '',
          actionsText: Array.isArray(issue.actions) ? issue.actions.join('、') : ''
        } : null
      };
    });
  },
  ...
  async loadKnowledge() {
    ...
    try {
      const res = await knowledgeService.getArticles({ useCache: true });
  ...
  async loadPlants() {
    ...
    try {
      const res = await knowledgeService.getPlantProfiles({ useCache: true });
      const plants = this.decoratePlants(Array.isArray(res?.plants) ? res.plants : []);
      this.setData({
        plants,
        plantLoading: false,
  ...
  togglePlant(e) {
    const plantId = String(e.currentTarget.dataset.id || '');
    this.setData({
      expandedPlantId: this.data.expandedPlantId === plantId ? '' : plantId
    });
  },

  copySource(e) {
    const sourceUrl = String(e.currentTarget.dataset.url || '').trim();
    if (!sourceUrl) {
