...
> 文档状态：Current Demo Snapshot  
> 核对日期：2026-07-19  
> 使用边界：本文解释当前下行链路；安全收口和命令状态机按实施路线 R1、R3 执行

> **安全提醒**：当前 `/device/cmd` 仍接受通用参数对象，不能把它直接暴露为 Agent 工具或正式用户的任意命令入口。R1 必须先收敛为业务动作白名单。
...
- 增加 `device_commands` 或等价持久化状态，支持审计、幂等、失败重试、ACK 和最终状态验证
- 在 R1 之前不提前建设复杂命令平台，但进入真实试用前必须完成动作白名单和审计
