# SCF + MySQL 迁移 Q&A（历史记录）

> 状态：历史迁移与排障记录，保留用于解释 CloudBase 到 SCF + MySQL 的演进过程。本文不是当前架构事实源，其中关于“尚无历史清理能力”等结论已经被后续仓库实现更新。最新状态请以 [当前架构与实现状态](./current-architecture.md) 为准。
