# SCF + MySQL 迁移阶段方案（历史快照）

> 状态：历史迁移资料，主要记录 2026-03 阶段从 CloudBase 切换到 SCF + MySQL 的决策。当前实现已经增加 `agent-scf`、Todo、植物库、植物日记、历史清理部署包和 EMQX 兼容代码。最新状态请以 [当前架构与实现状态](./current-architecture.md) 为准。

> 本文中的“当前”“尚未实现”等表述均应理解为该迁移阶段的状态，不代表 2026-07 的仓库现状。
