...
const cloudMediaFiles = ['deployment/cloud-initial/database/cloud-media.sql',
  'deployment/cloud-initial/database/cloudbase-media.sql'];

function mediaStatements() {
  return cloudMediaFiles.flatMap(file => splitSql(fs.readFileSync(path.join(projectRoot, file), 'utf8')));
}
...
export async function migrateDatabase(db, { mode = 'incremental', confirmDatabase } = {}) {
  ...
  try {
    ...
    if (mode !== 'verify') {
      ...
      const statements = mediaStatements();
      for (const statement of statements) {
        const addition = statement.match(/^ALTER TABLE cloud_media_objects ADD COLUMN (provider|upload_metadata)\b/i);
        if (addition && await column(db, 'cloud_media_objects', addition[1])) continue;
        const triggerName = statement.match(/^CREATE TRIGGER (nova_cloud(?:base)?_media_delete)\b/i)?.[1];
        if (triggerName) {
          const [triggers] = await db.execute(`SELECT EVENT_MANIPULATION, EVENT_OBJECT_TABLE, ACTION_TIMING, ACTION_STATEMENT
            FROM information_schema.TRIGGERS WHERE TRIGGER_SCHEMA = DATABASE() AND TRIGGER_NAME = ?`, [triggerName]);
          if (triggers.length) {
    ...
    await assertColumns(db, {
      ...
      cloud_media_objects: ['file_id', 'openid', 'object_key', 'bucket', 'region', 'sha256', 'state', 'cleanup_after', 'attempts', 'provider', 'upload_metadata']
    });
    ...
    for (const statement of mediaStatements().filter(value => /^CREATE TRIGGER /i.test(value))) {
      const name = statement.match(/^CREATE TRIGGER (\w+)/i)[1];
      const [triggers] = await db.execute(`SELECT ACTION_STATEMENT FROM information_schema.TRIGGERS
        WHERE TRIGGER_SCHEMA = DATABASE() AND TRIGGER_NAME = ?
        AND EVENT_MANIPULATION = 'DELETE' AND EVENT_OBJECT_TABLE = 'media_objects' AND ACTION_TIMING = 'AFTER'`, [name]);
      const expectedTrigger = statement.split(/FOR EACH ROW\s+/i)[1];
...
async function main() {
  ...
  const db = await mysql.createConnection({ host: process.env.DB_HOST, port: Number(process.env.DB_PORT) || 3306,
    ...
    ...(!local && process.env.DB_TLS_MODE === 'required' ? { ssl: { rejectUnauthorized: true, verifyIdentity: true, ...(process.env.DB_SSL_CA ? { ca: process.env.DB_SSL_CA.replace(/\\n/g, '\n') } : {}) } } : {}) });
