...
function main() {
  ...
  if (policy.handler !== 'index.main_handler') errors.push(`Handler 策略异常：${policy.handler}`);
  ...
  if (!fs.existsSync(scfManifestPath)) errors.push('缺少 scf-manifest.template.yaml');
    ...
    if (!/^\s*handler:\s*["']?index\.main_handler["']?\s*$/m.test(manifestText)) errors.push('SCF manifest handler 与原生入口不一致');
    if (/trigger:\s*["']?api-gateway/i.test(manifestText)) errors.push('SCF manifest 仍引用已停服的 API Gateway trigger');
  }
  if (!fs.existsSync(routeContractPath)) errors.push('缺少独立 Function URL 路由清单');
  ...
  const overlayPath = path.join(cloudRoot, 'overlays', 'cloud-runtime.js');
  if (!fs.existsSync(overlayPath)) errors.push('缺少云环境检查模块 overlays/cloud-runtime.js');
    ...
    if (!options.configPath) errors.push('--release 必须同时提供 --config <repo外或ignored配置>');
      ...
      if (!fs.existsSync(absoluteConfig)) errors.push(`配置文件不存在：${absoluteConfig}`);
        ...
        try {
          ...
          // Secret values are entered only in SCF; never reconstruct/dump them here.
