...
async function main() {
  for (const name of names) {
    assert.ok(fs.existsSync(path.join(builtRoot, name, 'index.js')), `Build ${name} first`);
    assert.match(fs.readFileSync(path.join(builtRoot, name, 'index.js'), 'utf8'), /validateInvocation/,
  ...
  try {
    ...
    await step('migration: empty database bootstrap, current schema and incremental replay', async () => {
      ...
      try {
        ...
        await connection.query('ALTER TABLE cloud_media_objects DROP COLUMN attempts, DROP COLUMN provider, DROP COLUMN upload_metadata');
        await connection.query('DROP TRIGGER nova_cloudbase_media_delete');
        await migrateDatabase(connection, { mode: 'incremental', confirmDatabase: databaseName });
    ...
    Object.assign(process.env, {
      DB_HOST: 'mysql.nova.invalid', DB_NAME: 'nova_staging_rehearsal', DB_PORT: '28245', DB_TLS_MODE: 'legacy-direct',
      NOVA_CLOUD_DEPLOYMENT: 'true',
      WECHAT_APPID: 'wx1234567890abcdef', LOCAL_MEDIA_ENABLED: 'false', LOCAL_DEV_AUTH_ENABLED: 'false',
      ALLOW_LEGACY_OPENID_FALLBACK: 'false', MEDIA_STORAGE_PROVIDER: 'cloudbase',
      CLOUDBASE_STORAGE_ENV_ID: 'nova-images-rehearsal', CLOUDBASE_STORAGE_REGION: 'ap-guangzhou',
      CLOUDBASE_STORAGE_PREFIX: 'nova-staging/rehearsal/media/', LLM_API_ENABLED: 'true',
    ...
    async function upload(purpose = 'conversation_image', extra = {}) {
      ...
      assert.equal(result.success, true, JSON.stringify(result)); assert.equal(result.upload.mode, 'cloudbase-client');
      providers.uploadCloudbase(result.upload, Buffer.from(extra.dataBase64 || imageBase64, 'base64'));
      const completed = await api('/media/complete', { fileId: result.upload.fileId }, tokenA);
      assert.equal(completed.success, true, JSON.stringify(completed)); assert.match(completed.media.fileId, /^cloud:\/\//);
      return completed.media.fileId;
    }
    ...
    await step('cloud media: empty-garden upload, owner resolve, byte integrity, native CloudBase, no database BLOB', async () => {
      photo = await upload();
      const [[row]] = await db.execute('SELECT provider, content_blob FROM media_objects WHERE file_id = ?', [photo]);
      assert.equal(row.provider, 'cloudbase'); assert.equal(row.content_blob, null);
      ...
      document = await upload('conversation_document', { originalName: 'notes.txt',
        dataBase64: Buffer.from('Rose care notes: provide ventilation. Check soil before watering.').toString('base64') });
    });
    ...
    await step('built document: CloudBase bytes -> real text parser -> persisted exchange + session context', async () => {
    ...
    await step('PDF and DOCX: real parsers from built packages, long MIME field and permanent conversation references', async () => {
      for (const fixture of documentFixtures()) {
        const uploaded = await upload('conversation_document', { originalName: fixture.name, dataBase64: fixture.bytes.toString('base64') });
        const result = await agent('/document/analyze', { sessionId: `formats-${fixture.name}`, mediaFileId: uploaded,
          message: '解释文档中的通风要求，不安排任务', clientTurnKey: `format-${fixture.name}` }, tokenA);
        assert.equal(result.success, true);
        const [[row]] = await db.execute('SELECT mime_type, content_blob, reference_type FROM media_objects WHERE file_id = ?', [uploaded]);
    ...
    await step('storage transactions: ROLLBACK restores metadata and cancellation does not delete the CloudBase file', async () => {
    ...
    await step('indeterminate native upload: lost client response keeps pending intent until safe expiry', async () => {
      const prepared = await api('/media/upload', { purpose: 'conversation_image', dataBase64: imageBase64 }, tokenA);
      providers.uploadCloudbase(prepared.upload, Buffer.from(imageBase64, 'base64'));
      // Simulate a lost upload response: the client cannot confirm and cancels.
      await api('/media/discard', { fileId: prepared.upload.fileId }, tokenA);
      const pending = await state(prepared.upload.fileId);
      assert.equal(pending.state, 'delete_pending'); assert.ok(pending.cleanup_after > new Date());
      assert.equal((await cloudMedia.cleanup(db, 2)).deleted, 0);
      await db.execute('UPDATE cloud_media_objects SET cleanup_after = DATE_SUB(NOW(), INTERVAL 1 MINUTE) WHERE file_id = ?', [pending.file_id]);
    ...
    await step('CloudBase image registration: native IDs, pending isolation, integrity, replay and cancellation', async () => {
      const prepared = await api('/media/upload', { purpose: 'conversation_image', dataBase64: imageBase64 }, tokenA);
      const grant = prepared.upload; assert.equal(grant.mode, 'cloudbase-client'); assert.equal(prepared.media, undefined);
      assert.equal((await state(grant.fileId)).state, 'pending');
      assert.equal((await api('/media/resolve', { fileIds: [grant.fileId] }, tokenA)).media.length, 0);
      assert.equal((await api('/media/complete', { fileId: grant.fileId }, tokenB, 404)).success, false);
      assert.equal((await api('/media/complete', { fileId: grant.fileId }, tokenA, 503)).success, false);
      providers.uploadCloudbase(grant, Buffer.from('wrong-image'));
      assert.equal((await api('/media/complete', { fileId: grant.fileId }, tokenA, 400)).success, false);
      assert.equal((await state(grant.fileId)).state, 'pending');
      providers.uploadCloudbase(grant, Buffer.from(imageBase64, 'base64'));
      for (let attempt = 0; attempt < 2; attempt++) {
        assert.equal((await api('/media/complete', { fileId: grant.fileId }, tokenA)).media.fileId, grant.fileId);
      }
      const [[registered]] = await db.execute('SELECT provider, content_blob FROM media_objects WHERE file_id = ?', [grant.fileId]);
      assert.equal(registered.provider, 'cloudbase'); assert.equal(registered.content_blob, null);
      assert.equal((await api('/media/resolve', { fileIds: [grant.fileId] }, tokenA)).media[0].contentBase64, imageBase64);
      assert.equal((await api('/media/resolve', { fileIds: [grant.fileId] }, tokenB)).media.length, 0);
      await api('/media/discard', { fileId: grant.fileId }, tokenB);
      assert.equal((await state(grant.fileId)).state, 'ready');
      const connection = await db.getConnection();
      try {
        await connection.beginTransaction();
        await connection.execute('DELETE FROM media_objects WHERE file_id = ?', [grant.fileId]);
        const [[queued]] = await connection.execute('SELECT state FROM cloud_media_objects WHERE file_id = ?', [grant.fileId]);
        assert.equal(queued.state, 'delete_pending'); await connection.rollback();
      } finally { connection.release(); }
      assert.equal((await state(grant.fileId)).state, 'ready');
      await api('/media/discard', { fileId: grant.fileId }, tokenA);
      providers.faults.delete = true;
      assert.equal((await cloudMedia.cleanup(db, 2)).failed, 1);
      assert.equal(Number((await state(grant.fileId)).attempts), 1);
      await db.execute('UPDATE cloud_media_objects SET cleanup_after = DATE_SUB(NOW(), INTERVAL 1 MINUTE) WHERE file_id = ?', [grant.fileId]);
      assert.equal((await cloudMedia.cleanup(db, 2)).deleted, 1);
      const cancelled = (await api('/media/upload', { purpose: 'conversation_image', dataBase64: imageBase64 }, tokenA)).upload;
      await api('/media/discard', { fileId: cancelled.fileId }, tokenA);
      providers.uploadCloudbase(cancelled, Buffer.from(imageBase64, 'base64'));
      assert.equal((await api('/media/complete', { fileId: cancelled.fileId }, tokenA, 404)).success, false);
      assert.equal((await cloudMedia.cleanup(db, 2)).deleted, 0);
      await db.execute('UPDATE cloud_media_objects SET cleanup_after = DATE_SUB(NOW(), INTERVAL 1 MINUTE) WHERE file_id = ?', [cancelled.fileId]);
      assert.equal((await cloudMedia.cleanup(db, 2)).deleted, 1);
      assert.equal(providers.objects.has(cancelled.cloudPath), false);
    });
    async function uploadCloudImage(purpose, extra = {}) {
      const { upload: grant } = await api('/media/upload', { purpose, dataBase64: imageBase64, ...extra }, tokenA);
      providers.uploadCloudbase(grant, Buffer.from(imageBase64, 'base64'));
      const response = await api('/media/complete', { fileId: grant.fileId }, tokenA);
      assert.equal(response.success, true); return grant.fileId;
    }
    await step('CloudBase images: avatar, plant cover, journal and Vision use existing business claims; documents use the same CloudBase path', async () => {
      const avatar = await uploadCloudImage('profile_avatar');
      assert.equal((await api('/user/profile', { nickname: '图片混合路线', avatarFileId: avatar }, tokenA)).success, true);
      assert.equal((await api('/user/profile', { nickname: '不可借图', avatarFileId: avatar }, tokenB)).success, false);
      const cover = await uploadCloudImage('plant_cover');
      const imagePet = (await api('/plant/pet-create', { nickname: '云开发图片植宠', coverFileId: cover }, tokenA)).pet;
      assert.ok(imagePet.id);
      const journalPhoto = await uploadCloudImage('journal_photo', { plantPetId: imagePet.id });
      const journal = await api('/journal/create', { plantPetId: imagePet.id, eventType: 'note', title: '图片记录',
        photoFileIds: [journalPhoto] }, tokenA);
      assert.equal(journal.success, true, JSON.stringify(journal));
      const image = await uploadCloudImage('conversation_image', { plantPetId: imagePet.id });
      const vision = await agent('/vision/analyze', { sessionId: 'cloudbase-image', plantPetId: imagePet.id,
        mediaFileId: image, imageBase64, mimeType: 'image/png', message: '只介绍，不安排任务', clientTurnKey: 'cloudbase-image-turn' }, tokenA);
      assert.equal(vision.success, true);
      const turn = await agent('/agent/chat', { sessionId: 'cloudbase-image', plantPetId: imagePet.id,
        message: '继续聊', clientTurnKey: 'cloudbase-next-turn' }, tokenA);
      const fork = (await agent('/agent/conversation-fork', { sourceMessageId: turn.userMessageId }, tokenA)).conversation;
      assert.equal(fork.copiedMessageCount, 2);
      for (const id of [turn.userMessageId, vision.userMessageId]) {
        assert.equal((await agent('/agent/conversation-withdraw', { sessionId: 'cloudbase-image', userMessageId: id }, tokenA)).success, true);
      }
      assert.equal((await state(image)).state, 'ready');
      const loaded = await agent('/agent/session', { sessionId: fork.sessionId, plantPetId: imagePet.id }, tokenA);
      const copied = loaded.messages.find(item => item.role === 'user');
      assert.equal((await agent('/agent/conversation-withdraw', { sessionId: fork.sessionId, userMessageId: copied.id }, tokenA)).success, true);
      assert.equal((await state(image)).state, 'delete_pending');
      await cloudMedia.cleanup(db, 2);
      for (const fixture of documentFixtures()) {
        const documentId = await upload('conversation_document', { originalName: fixture.name,
          dataBase64: fixture.bytes.toString('base64') });
        assert.equal((await state(documentId)).provider, 'cloudbase');
        const analyzed = await agent('/document/analyze', { sessionId: 'hybrid-' + fixture.name, mediaFileId: documentId,
          message: '解释文档，不安排任务', clientTurnKey: 'hybrid-' + fixture.name }, tokenA);
        assert.equal(analyzed.success, true);
      }
      assert.equal((await api('/plant/pet-delete', { plantPetId: imagePet.id }, tokenA)).success, true);
      assert.equal((await state(cover)).state, 'delete_pending'); assert.equal((await state(journalPhoto)).state, 'delete_pending');
      assert.equal((await state(avatar)).state, 'ready');
      assert.equal((await cloudMedia.cleanup(db, 2)).deleted, 2);
      const replacement = await uploadCloudImage('profile_avatar');
      await api('/user/profile', { avatarFileId: replacement }, tokenA);
      assert.equal((await state(avatar)).state, 'delete_pending');
      assert.equal((await cloudMedia.cleanup(db, 2)).deleted, 1);
    });
    await step('negative gates and unchanged external boundary', async () => {
      process.env.CLOUDBASE_STORAGE_ENV_ID = '';
      assert.equal((await api('/plant/pets', {}, tokenA, 503)).code, 'CLOUD_CONFIGURATION_BLOCKED');
      process.env.CLOUDBASE_STORAGE_ENV_ID = 'nova-images-rehearsal';
      ...
      assert.equal(poolAssertions, names.length);
      assert.ok(providers.calls.token > 0); assert.ok(providers.calls.cloudbase.length > 10);
    });
    console.log(JSON.stringify({ status: 'PASS', evidence: 'LOCAL_BUILT_CLOUD_CANDIDATE_WITH_REAL_ISOLATED_MYSQL_AND_EXTERNAL_PROVIDER_STUBS',
      ...
      substitutes: ['WeChat code exchange response', 'LLM and Vision response', 'WeChat CloudBase file HTTP APIs and native client upload transport'],
      realPaths: ['built original index.main_handler and SCF business routes', 'MySQL transactions and trigger', 'session/summary persistence', 'owner/JWT checks', 'task confirmation/idempotency'],
