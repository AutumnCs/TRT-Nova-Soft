...
try {
  ...
  Invoke-Check 'cloud-unit-and-http' $NodeExecutable (@('--test') + $testFiles)
  Invoke-Check 'original-package-entry' $NodeExecutable @('deployment/cloud-initial/tools/smoke-built-packages.mjs')
  $previousBuiltTest = $env:NOVA_TEST_USE_BUILT
  try {
    $env:NOVA_TEST_USE_BUILT = 'true'
    Invoke-Check 'built-wechat-http' $NodeExecutable @('--test', 'deployment/cloud-initial/test/cloud-sdk-transport.test.mjs')
  } finally { $env:NOVA_TEST_USE_BUILT = $previousBuiltTest }
