...
### UI improvements: prototype before mini program

User-approved workflow (2026-09-10): understand the requirement, use design-taste-frontend to establish a coherent design direction, then build an independent interactive HTML prototype. Iterate through testing, hands-on use and feedback until the user is satisfied. Apply it to the mini program only after explicit user approval, then repeat testing and feedback in the mini program and on phones. A bug fix does not authorize an unrelated visual redesign. See [UI taste and prototype workflow](./ui-taste-and-prototype-workflow.md) for the meeting-derived design requirements.
