...
async function fetchWechatSession(code) {
  ...
  const result = await new Promise((resolve, reject) => {
    let timer;
    const fail = () => { clearTimeout(timer); reject(new Error('微信登录服务暂时不可用，请稍后重试')); };
    const request = https.get(url, (res) => {
        ...
        res.on('data', (chunk) => {
          raw += chunk;
          if (Buffer.byteLength(raw, 'utf8') > 65536) { request.destroy(); fail(); }
        });
        res.on('error', fail);
        res.on('end', () => {
          clearTimeout(timer);
    ...
    // An absolute deadline, not just a socket-idle timeout. Do not log the URL:
    // code2Session requires the AppSecret in its query string.
    timer = setTimeout(() => { request.destroy(); fail(); }, 8000);
  });
...
exports.main = async (event) => {
  if (process.env.NOVA_CLOUD_DEPLOYMENT === 'true') {
    const blocked = require('./cloud-runtime').validateInvocation('auth-scf', event);
