...
test('永久媒体标识接受 local、cloud与HTTPS，不再接受COS，拒绝临时路径', () => {
  assert.equal(normalizePersistentFileId('cos://12345678-1234-1234-1234-123456789abc'), '');
...
test('云端认领允许没有附件，不允许外部URL绕过附件归属校验', async () => {
  ...
  process.env.MEDIA_STORAGE_PROVIDER = 'cloudbase';
  try {
...
test('云端按单附件解析限制返回大小，不批量返回超出SCF载荷的图片字节', async () => {
  ...
  try {
    ...
    const result = await resolveLocalMediaForUser({}, 'owner', { fileIds: [
      'cloud://test-env.bucket/file-a.png', 'cloud://test-env.bucket/file-b.png'
    ] });
