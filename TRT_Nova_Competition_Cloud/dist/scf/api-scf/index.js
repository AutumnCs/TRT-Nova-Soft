 ...
 * - POST /care/tasks
 * - POST /care/task
 * - POST /care/task-proposal
 * - POST /care/task-proposal-confirm
 * - POST /care/task-proposal-cancel
 * - POST /care/task-create
 * - POST /care/task-update
 * - POST /care/task-postpone
 * - POST /care/task-delete
 * - POST /care/task-complete
 * - GET /care/summary
 * - POST /care/events
 * - GET /weather/summary
 * - POST /weather/cities
 * - POST /weather/preference
 * - POST /weather/preference-clear
 * - POST /ai/diagnoses
 * - POST /ai/diagnosis-save
 * - POST /ai/diagnosis-correct
 * - POST /ai/diagnosis-delete
 * - POST /ai/memories
 * - POST /ai/memory-create
 * - POST /ai/memory-update
 * - POST /ai/memory-delete
 * - POST /ai/memory-clear
 * - POST /ai/memory-proposal
 * - POST /ai/memory-proposal-confirm
 * - POST /ai/memory-proposal-dismiss
 *
...
async function getDb() {
  ...
  pool = mysql.createPool({
    ...
    ...(process.env.DB_TLS_MODE === 'required' ? { ssl: { rejectUnauthorized: true, verifyIdentity: true,
...
async function bindDeviceForUser(db, openid, input) {
  ...
  try {
    ...
    // Restore the remote baseline: some existing devices have no Nova_ prefix.
    if (!device || (device.status && device.status !== 'active')) {
      ...
      if (fallback && (!fallback.status || fallback.status === 'active')) device = fallback;
    }
    if (!device || (device.status && device.status !== 'active')) {
      await conn.rollback();
      return {
...
async function getUserProfile(db, openid) {
  const [rows] = await db.execute(
    `SELECT openid, unionid, nick_name, avatar_url, avatar_file_id, gender, birthday, region_json,
...
async function saveUserProfile(db, openid, input) {
  const now = toSqlDateTime(Date.now());
  const avatarFileIdInput = typeof input?.avatarFileId === 'string' ? input.avatarFileId.trim() : '';
  const avatarFileId = normalizePersistentFileId(avatarFileIdInput);
  if (avatarFileIdInput && !avatarFileId) {
    return { success: false, msg: '头像图片标识无效' };
  }
  const payload = {
  ...
  try {
    await withTransaction(db, async (connection) => {
      const [currentRows] = await connection.execute(
        'SELECT avatar_file_id FROM users WHERE openid = ? LIMIT 1 FOR UPDATE',
        [openid]
      );
      const oldAvatarFileId = currentRows[0]?.avatar_file_id || '';
      const nextAvatarFileId = Object.prototype.hasOwnProperty.call(input || {}, 'avatarFileId')
        ? payload.avatarFileId
        : oldAvatarFileId;

      await connection.execute(
        `INSERT INTO users
          (openid, nick_name, avatar_url, avatar_file_id, gender, birthday, region_json,
           experience_level, signature, phone, email, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ...
      if (nextAvatarFileId !== oldAvatarFileId) {
        const claimed = await claimLocalMediaFiles(connection, openid, [nextAvatarFileId], {
          purpose: 'profile_avatar',
          referenceType: 'profile_avatar',
          referenceKey: openid
        });
        if (!claimed.success) {
          const error = new Error(claimed.msg);
          error.isProfileDomainError = true;
          throw error;
        }
        await deleteLocalMediaFiles(connection, openid, [oldAvatarFileId], {
          referenceType: 'profile_avatar',
          referenceKey: openid
        });
      }
    ...
    if (err?.isProfileDomainError) return { success: false, msg: err.message };
    throw err;
  }
...
exports.main = async (event) => {
  if (process.env.NOVA_CLOUD_DEPLOYMENT === 'true') {
    const blocked = require('./cloud-runtime').validateInvocation('api-scf', event);
  ...
  try {
    ...
    if (method === 'GET' && path.endsWith('/weather/summary')) {
      return json(200, await getWeatherSummaryForUser(db, openid));
    }

    if (method === 'POST' && path.endsWith('/weather/cities')) {
      return json(200, await searchWeatherCities(body));
    }

    if (method === 'POST' && path.endsWith('/weather/preference')) {
      return json(200, await saveWeatherPreferenceForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/weather/preference-clear')) {
      return json(200, await clearWeatherPreferenceForUser(db, openid));
    }

    if (method === 'POST' && path.endsWith('/ai/diagnoses')) {
      return json(200, await listDiagnosesForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/ai/diagnosis-save')) {
      return json(200, await saveDiagnosisForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/ai/diagnosis-correct')) {
      return json(200, await correctDiagnosisForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/ai/diagnosis-delete')) {
      return json(200, await deleteDiagnosisForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/ai/memories')) {
      return json(200, await listMemoriesForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/ai/memory-create')) {
      return json(200, await createMemoryForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/ai/memory-update')) {
      return json(200, await updateMemoryForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/ai/memory-delete')) {
      return json(200, await deleteMemoryForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/ai/memory-clear')) {
      return json(200, await clearMemoriesForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/ai/memory-proposal')) {
      return json(200, await getActionProposalForUser(db, openid, body, 'memory_preference'));
    }

    if (method === 'POST' && path.endsWith('/ai/memory-proposal-confirm')) {
      return json(200, await confirmMemoryProposalForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/ai/memory-proposal-dismiss')) {
      return json(200, await dismissActionProposalForUser(db, openid, body, 'memory_preference'));
    }
    ...
    if (method === 'POST' && path.endsWith('/knowledge/plants')) {
      return json(200, await listKnowledgePlants(db, body));
    }

    if (method === 'POST' && path.endsWith('/knowledge/plant')) {
      return json(200, await getKnowledgePlant(db, body));
    }

    if (method === 'POST' && path.endsWith('/care/tasks')) {
      return json(200, await listCareTasksForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/care/task')) {
      return json(200, await getCareTaskForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/care/task-proposal')) {
      return json(200, await getCareTaskProposalForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/care/task-proposal-confirm')) {
      return json(200, await confirmCareTaskProposalForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/care/task-proposal-cancel')) {
      return json(200, await cancelCareTaskProposalForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/care/task-create')) {
      return json(200, await createCareTaskForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/care/task-update')) {
      return json(200, await updateCareTaskForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/care/task-postpone')) {
      return json(200, await postponeCareTaskForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/care/task-delete')) {
      return json(200, await deleteCareTaskForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/care/task-complete')) {
      return json(200, await completeCareTaskForUser(db, openid, body));
    }

    if (method === 'GET' && path.endsWith('/care/summary')) {
      return json(200, await getCareSummaryForUser(db, openid));
    }

    if (method === 'POST' && path.endsWith('/care/events')) {
      return json(200, await listCareEventsForUser(db, openid, body));
    }
    ...
    if (method === 'POST' && path.endsWith('/journal/timeline')) {
      return json(200, await listJournalTimelineForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/journal/record')) {
      return json(200, await getJournalRecordForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/journal/create')) {
      return json(200, await createJournalRecordForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/journal/update')) {
      return json(200, await updateJournalRecordForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/journal/delete')) {
      return json(200, await deleteJournalRecordForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/media/upload')) {
      return json(200, await uploadLocalMediaForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/media/resolve')) {
      return json(200, await resolveLocalMediaForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/media/complete')) {
      try {
        return json(200, await completeMediaUploadForUser(db, openid, body));
      } catch (error) {
        if ([400, 404, 409].includes(error.statusCode)) {
          return json(error.statusCode, { success: false, msg: error.message });
        }
        throw error;
      }
    }

    if (method === 'POST' && path.endsWith('/media/discard')) {
      return json(200, await discardLocalMediaForUser(db, openid, body));
    }
    ...
    if (method === 'POST' && path.endsWith('/plant/pets')) {
      return json(200, await listPlantPetsForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/plant/pet')) {
      return json(200, await getPlantPetForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/plant/pet-create')) {
      return json(200, await createPlantPetForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/plant/pet-update')) {
      return json(200, await updatePlantPetForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/plant/pet-archive')) {
      return json(200, await archivePlantPetForUser(db, openid, body));
    }

    if (method === 'POST' && path.endsWith('/plant/pet-delete')) {
      return json(200, await deletePlantPetForUser(db, openid, body));
    }
    ...
    // Preserve intentional service-unavailable responses from the cloud media boundary.
    // Other unexpected failures keep the existing 500 contract.
    return json(isAuthError ? 401 : (err.statusCode === 503 ? 503 : 500), {
      success: false,
...
exports._private = { bindDeviceForUser };
