...
exports.main = async (event = {}) => {
  if (process.env.NOVA_CLOUD_DEPLOYMENT === 'true') {
    const blocked = require('./cloud-runtime').validateInvocation('history-cleanup-scf', event);
    if (blocked) return blocked;
  }
  try {
    const db = await getDb();
    const retention = getRetentionDays();
    const cloudMedia = process.env.MEDIA_STORAGE_PROVIDER === 'cloudbase'
      ? await require('./cloud-media').cleanup(db, 2) : null;
