...
> **🆕 更新说明 v3.1.0**:
> - ✅ Runtime + Skills 架构升级（43个模块 + AgentCore V2 适配层）
> - ✅ 六层记忆系统重构（L1-L6 分层架构）
> - ✅ WorkBuddy 风格 UI 全面升级 + 流式响应支持
> - ✅ Markdown 渲染支持（AI回复支持标题、列表、代码块、表格等格式）
> - ✅ 深度思考模式（更深入的分析与推理）
> - ✅ 输入框粘贴图片功能（剪贴板图片直接粘贴 + 缩略图预览）
> - ✅ 流式聊天优化（httpx 直接调用，修复输入法兼容问题）
...
**功能特性**:
- 📋 **剪贴板粘贴**: Ctrl/Cmd+V 直接粘贴截图或复制的图片
- 🖼️ **缩略图预览**: 粘贴/上传的图片显示缩略图预览
- 🔍 **点击查看大图**: 点击缩略图在新标签页查看原图
- ✂️ **一键删除**: 支持删除已粘贴的图片附件
- 📎 **附件上传**: 支持图片、PDF、TXT、Word 等多种文件格式
- 💬 **附件分析**: 上传附件后自动切换为内容分析模式
...
│   │   └── README.md                # Agent模块文档
│   │
│   ├── runtime/                      # ✅ Runtime+Skills 架构（v3.1.0）
│   │   ├── conversation/            # 对话运行时
│   │   ├── skills/                  # 技能注册（5大核心技能）
│   │   │   ├── emotion_skill.py     # 情感技能
│   │   │   ├── memory_skill.py      # 记忆技能
│   │   │   ├── planning_skill.py    # 规划技能
│   │   │   ├── reflect_skill.py     # 反思技能
│   │   │   └── tool_skill.py        # 工具技能
│   │   ├── policy/                  # 策略引擎
│   │   ├── session/                 # 会话状态机
│   │   ├── hooks/                   # 钩子分发
...
│   │   │   ├── ChatContainer.js     # 聊天容器组件（含图片粘贴）
│   │   │   ├── FeedbackModal.js     # 反馈模态框
│   │   │   ├── MarkdownRenderer.js  # Markdown渲染组件
│   │   │   ├── MultimodalInput.js   # 多模态输入组件
│   │   │   ├── PersonalizationPanel.js # 个性化面板
│   │   │   ├── SkillsPanel.js       # 技能中心面板
│   │   │   ├── HistoryManagementModal.js # 历史管理
│   │   │   └── Sidebar.js           # 侧边栏组件
│   │   ├── services/                # 服务层
│   │   │   └── ChatAPI.js           # API调用封装（含流式响应）
│   │   ├── hooks/                   # React Hooks
│   │   │   ├── useChat.js           # 聊天逻辑（流式消息）
│   │   │   ├── useFileUpload.js     # 文件上传（含图片粘贴）
│   │   │   ├── useSession.js        # 会话管理
│   │   │   ├── useFeedback.js       # 反馈管理
│   │   │   └── useURLDetection.js   # URL检测
│   │   ├── utils/                   # 工具函数
│   │   ├── styles/                  # 样式文件（含input.js输入框样式）
│   │   └── constants/               # 常量定义
│   ├── package.json                 
│   └── package-lock.json            
│
...
| Runtime+Skills | `backend/runtime/` | ✅ Runtime+Skills架构（43模块, 5大技能, 策略引擎） |
...
现代化的 WorkBuddy 风格聊天界面设计，包含：
- 🎭 **情感标签可视化** - 实时显示AI识别的情感状态
- 💬 **建议按钮快速回复** - 智能推荐回复选项
- ⌨️ **打字机效果** - 根据情绪动态调整打字速度和光标颜色
- 📝 **Markdown渲染** - AI回复支持标题、列表、代码块、表格等格式
- 🖼️ **图片粘贴上传** - 输入框支持直接粘贴剪贴板图片
- 💡 **深度思考模式** - 一键开启更深入的分析与推理
...
- [ ] 用户偏好学习和个性化推荐
- [x] ~~多模态支持（图片理解）~~ ✅ v3.1.0 已完成
- [x] ~~语音转文字集成~~ ✅ v3.0.0 已完成
- [ ] 性能优化和缓存策略
...
### v3.1.0 (2025-05-14) - 当前版本
- ✅ Runtime + Skills 架构升级（43个模块 + AgentCore V2 适配层）
- ✅ 六层记忆系统重构（L1-L6 分层架构）
- ✅ WorkBuddy 风格 UI 全面升级 + 流式响应支持
- ✅ Markdown 渲染组件（react-markdown + remark-gfm）
- ✅ 深度思考模式（更深入的分析与推理）
- ✅ 输入框粘贴图片功能（剪贴板图片直接粘贴 + 缩略图预览）
- ✅ 流式聊天优化（httpx 直接调用，修复输入法兼容问题）
- ✅ 附件上传智能模式切换（自动切换为内容分析模式）
- ✅ PDF 解析错误处理优化
...
**心语情感陪伴机器人 v3.1.0** - 从被动响应到主动服务，从线性工作流到 Runtime+Skills 架构，我们一直在进化。
